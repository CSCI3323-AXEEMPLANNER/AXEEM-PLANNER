...
export function CLASS_OBJECT_CREATION(class_ID, name, desc, professor_ID, date, time) {
    ...
    // LIKELY ADD PREVENTION OF ADDITION IF ATLEAST TITLE OR DESCRIPTION IS EMPTY
    const obj = {'class_ID': class_ID, 'name': name, "desc": desc, "professor_ID": professor_ID, 'date': (date === undefined) ? new Date().toLocaleDateString() : date, 'time': (time === undefined) ? new Date().toLocaleTimeString() : time}
    return obj;
};